...
class BaseTrainer:
    ...
    def __init__(self, generator, discriminator, criterion, metrics,
    ...
    def _save_checkpoint(self, epoch, save_best=False, only_best=False):
        ...
        :param save_best: if True, rename the saved checkpoint to
            'model_best.pth'
        """
        archG = type(self.generator).__name__
        stateG = {
            "arch": archG,
            "epoch": epoch,
        ...
        if save_best:
            ...
            torch.save(stateG, best_path)
            self.logger.info("Saving current best:generator_best.pth ...")
        if self.discriminator is not None:
            archD = type(self.discriminator).__name__
            stateD = {
                "arch": archD,
                "epoch": epoch,
            ...
            if not (only_best and save_best):
                ...
                self.logger.info("Saving checkpoint: {} ...".format(filename))
            if save_best:
                best_path = str(self.checkpoint_dir / "discriminator_best.pth")
                torch.save(stateD, best_path)
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        """
        parent_path = resume_path.parent.absolute()
        name = resume_path.name
        discriminator_path = parent_path / name.replace("generator",
                                                        "discriminator")
        ...
        if checkpoint["config"]["archG"] != self.config["archG"]:
            self.logger.warning(
        ...
        # not changed.
        if (
                checkpoint["config"]["optimizerG"]["type"] != self.config[
                "optimizerG"]["type"] or
                checkpoint["config"]["optimizerG"]["args"] != self.config[
                "optimizerG"]["args"] or
        ...
        if discriminator_path.exists():
            resume_path = str(discriminator_path)
            self.logger.info("Loading checkpoint: {} ...".format(resume_path))
            checkpoint = torch.load(resume_path, self.device)

            # load architecture params from checkpoint.
            if checkpoint["config"]["archD"] != self.config["archD"]:
                self.logger.warning(
            ...
            # not changed.
            if (
                    checkpoint["config"]["optimizerD"]["type"] != self.config[
                    "optimizerD"]["type"] or
                    checkpoint["config"]["optimizerD"]["args"] != self.config[
                    "optimizerD"]["args"] or
                ...
                self.logger.warning(
                    "Warning: Optimizer or lr_scheduler given in config file "
                    "is different from that of checkpoint. Optimizer "
                    "parameters not being resumed."
                )
            else:
                self.optimizerD.load_state_dict(checkpoint["optimizer"])
